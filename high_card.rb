...
#######################################################################
# list_winners
# Description: Print out winner name list(s)
#     Parmeters:  winners- array of strings of player names
#
#     Return - None
#######################################################################
def list_winners (winners_list)

  for w in winners_list

    # handle case of more than one winner but not last winner
    if (winners_list.length > 1) && ( w != winners_list.last) && (w != winners_list[0] )
      sep = ", "
    elsif (winners_list.length > 1) && ( w == winners_list.last)
      sep =" and "
    end # if
    print "#{sep}"
    print "#{w}"          # print name
  end #for w
end # function list_winners

#######################################################################
# list_cards
# Description: Print card values for each winner
#     Parmeters:  highcard - value of winning card
#                 Game - Array of all players in game
#     Return - None
#######################################################################

def list_cards( winners,game)
  for c in winners
    print_card(c,game)
    puts ""  # force new line
  end # for c
